...
for idx, class_name in enumerate(df['ClassName']):
    ...
    ax.plot(angles, values, 'o-', linewidth=2, color=['#3B82F6', '#10B981', '#EF4444', '#F59E0B'][idx % 4])
    ax.fill(angles, values, alpha=0.25, color=['#3B82F6', '#10B981', '#EF4444', '#F59E0B'][idx % 4])
...
sizes = [max(attr * 100, 50) for attr in df['NumberOfAttributes'].tolist()]
